...
init_Grid = function()
{
	$("#gridUnit").jqGrid(
	{
		...
		{
			id : "unitID"					/* 数据的主键id。如果不设置，默认以"id"属性作为主键id */
		},
		...
		beforeProcessing : function(data, status, xhr)
		{
			 ...
			 * 转换leaf、处理自动展开
			 */
			var treeReader = $("#gridUnit").getGridParam("treeReader");
			mc.grid.TreeDataParseLeafLevel(data.data, treeReader, "isLeaf", 1);

			G_UnitDataStore.load("unitID", data.data);
		},
		treeGridModel: "adjacency", 	/* 数据模式，一般采用adjacecy邻接模式，依靠父子id建立关系。 */
		ExpandColumn : "unitCode",		/* 在【编码】列展现树形结构 */
...
init_Panel = function()
{
	...
	$("#btnRefresh").click(reloadUnitGrid);
...
reloadUnitGrid = function()
{
	$("#gridUnit").setGridParam(
	{
		datatype : "json",
		treedatatype : "json",
		postData : {}
...
btnAddEvent = function(event)
{
	...
	if ($("#gridUnit").isSelect())
	{
		var selectID = $("#gridUnit").getId();
		var data = G_UnitDataStore.get(selectID);

		initData.parentID = data.unitID;
		initData.parentCode = data.unitCode;
		initData.parentName = data.unitName;
	...
	$("#dialogUintEdit").UnitEditDialog("initData", "add", initData);
	$("#dialogUintEdit").UnitEditDialog("open");
};
...
btnUpdateEvent = function(event)
{
	...
	var unitId = $("#gridUnit").getId();
	var unit = G_UnitDataStore.get(unitId);

	$("#dialogUintEdit").UnitEditDialog("initData", "update", unit);
	$("#dialogUintEdit").UnitEditDialog("open");
}
...
btnDeleteEvent = function(event)
{
	...
	if (unit.isLeaf != 1)
	{
		mc.alert("公司【" + unit.unitName + "】还拥有下级公司，不能删除");
		return;
	}

	if (unit.status == 1)
	{
		mc.alert("公司【" + unit.unitName + "】还未停用，不能删除");
		return;
	}

	mc.confirm("如果公司已发生过业务数据，则不能删除。<br>请确认是否要删除公司【" + unit.unitName + "】？", function(result)
	{
		if (! result)
		{
			return;
...
{
	if ($("#dialogUintEdit").UnitEditDialog("isOpen"))
	{
		mc.alert("请先关闭对话框");
		return false;
	}
