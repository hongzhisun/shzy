...
(function($)
{
	$.widget("sm.UnitEditDialog", $.mc.BaseDialog,
	{
		/**
		 * 默认初始化参数
		 */
		...
		{
			title : "公司编辑",
			content_url : "mc/sm/basedata/unit/unit_edit_dialog.html",			/* 组件html url */
			height : 340,
			width : 670,
			resize : false,
			maxmin : false,
		...
		initData : function(edit_mode, data)
		{
			...
			if (this.edit_mode == "add")
			{
				...
				this.options.dialog.title = "公司 - 新增";
			}
			else
			{
				$("#unitCode").val(this.entitydata.unitCode);
				$("#unitName").val(this.entitydata.unitName);
				...
				this.options.dialog.title = "公司 - 修改";
			}
		},
		/**
		 * @override
