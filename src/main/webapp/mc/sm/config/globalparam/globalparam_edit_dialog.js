...
(function($)
{
	$.widget("sm.GlobalParamEditDialog", $.mc.BaseDialog,
	{
		/**
		 * 默认初始化参数
		 */
		...
		{
			title : "全局参数",
			content_url : "mc/sm/config/globalparam/globalparam_edit_dialog.html",
			height : 280,
			width : 600,
			resize : false,
			maxmin : false,
		...
		initData : function(edit_mode, data)
		{
			...
			if (this.edit_mode == "add")
			{
				...
				this.options.dialog.title = "新增 - 全局参数";
			}
			else
			{
				...
				this.options.dialog.title = "修改 - 全局参数";
			}
		},
		/**
		 * @override
