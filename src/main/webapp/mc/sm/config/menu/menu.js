...
init_Grid = function()
{
	$("#gridMenu").jqGrid(
	{
		...
		beforeProcessing : function(data, status, xhr)
		{
			...
			G_MenuDataStore.load("id", resultArray);
...
reloadGrid = function()
{
	if (! $("#fieldModule").ModuleTreeField("isSelect"))
	{
		mc.alert("请先选择模块");
		return;
	}

	var moduleid = $("#fieldModule").ModuleTreeField("id");

	$("#gridMenu").setGridParam(
	{
		datatype : "json",
		treedatatype : "json",
		postData :
...
btnAddEvent = function(event)
{
	...
	if ($("#gridMenu").isSelect())
	{
		var selectID = $("#gridMenu").getId();
		var data = G_MenuDataStore.get(selectID);
		if (data.level >= 2)
		{
	...
	$("#dialogMenuEdit").MenuEditDialog("initData", "add", initData);
	$("#dialogMenuEdit").MenuEditDialog("open");
};
...
btnUpdateEvent = function(event)
{
	...
	$("#dialogMenuEdit").MenuEditDialog("initData", "update", data);
	$("#dialogMenuEdit").MenuEditDialog("open");
}
...
btnDeleteEvent = function(event)
{
	...
	if (data.internal == 1)
	{
		mc.msg("菜单【" + data.name + "】是系统内置菜单，不可删除");
		return;
	}

	mc.confirm("请确认是否要删除菜单【" + data.name + "】？", function(result)
	{
		if (! result)
		{
			return;
...
{
	if ($("#dialogMenuEdit").MenuEditDialog("isOpen"))
	{
		mc.alert("请先关闭对话框");
		return false;
	}
