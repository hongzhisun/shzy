...
(function($)
{
	$.widget("sm.ModuleEditDialog", $.mc.BaseDialog,
	{
		/**
		 * 默认初始化参数
		 */
		...
		{
			title : "模块",
			content_url : "mc/sm/config/module/module_edit_dialog.html",			/* 组件html url */
			height : 380,
			width : 670,
			resize : false,
			maxmin : false,
		...
		initData : function(edit_mode, data)
		{
			...
			if (this.edit_mode == "add")
			{
				...
				this.options.dialog.title = "新增 - 模块";
			}
			else
			{
				...
				this.options.dialog.title = "修改 - 模块";
			}
		},
		_createDialogOptions : function()
		{
			...
			dialogOptions.success = $.proxy(function($dom, index)
			{
				...
				if (this.entitydata.internal == 1)
				{
					mc.msg("模块【" + this.entitydata.name + "】是系统内置模块，只可修改名称、显示、排序、页面url等部分信息");
				}
			}, this);
