 ...
 * @class
 * 基础列表选择对话框<br>
 * @extends ssc.component.BaseListField
 * 一、支持特性：<br>
 ...
 * xy_ClearClickEvent	点击"清除"按钮触发事件
 * xy_ValueChangeEvent	选择项变化事件
 * 实现abstract函数
 * prepareBaseParams	按照ssc.common.BaseCondition格式拼写查询条件，
 * 						以便在提交查询action参数时一起提交。
 * baseBeforeClickValid	点“选择”按钮前，验证方法（不支持提示框）
 * 
 * xy_beforeSelectValid			点击“选择”按钮前，验证方法。(支持提示框)
 * xy_beforeSelectValidPass		xy_beforeSelectValid验证通过后，调用该方法
 * 
 * xy_beforeClearValid			点击“清除”按钮前，验证方法。(支持提示框)
 * xy_beforeClearValidPass		xy_beforeClearValid验证通过后，调用该方法
 * 
 ...
 */
ssc.component.BaseListField = Ext.extend(Ext.form.TwinTriggerField,
{
	/* 关键属性，必须保留 */
	allowBlur : true,
	...
	initComponent : function()
	{
		...
		ssc.component.BaseListField.superclass.initComponent.call(this);
	...
	btn_OpenWinEvent : function()
	{
		...
		{
			this.xy_beforeSelectValid.call(this.xy_ParentObjHandle, this);
	...
	clickSelectButtonProcess : function()
	{
		...
		{
			if(this.m_Dialog.m_searchField != null)
			{
				this.m_Dialog.m_searchField.onTrigger2Click();
			}
			this.m_blDialogSelected = false;
		}
	...
	{
		this.m_Dialog = new ssc.component.BaseListDialog(
		{
			...
			xy_ColumnConfig : this.xy_ColumnConfig,
			xy_Filter : this.xy_Filter,
			...
			xy_FieldList : this.xy_FieldList,
			xy_KeyField : this.xy_KeyField,
			xy_DisplayField : this.xy_DisplayField,
...
/* 2013-02-25 判断值是否有变化
	...
	getValue : function()
	{
		...
		{
			return ssc.component.BaseListField.superclass.getValue.call(this);
		}
...
/* ==================== */
	 ...
	 * 如复选，则返回TreeNode.id的字符串，逗号分隔<br>
	 * 不会查找未加载的节点<br>
	 */
	 ...
	 * 如复选，则返回TreeNode.text的字符串，逗号分隔<br>
	 * 不会查找未加载的节点<br>
	 */
	 ...
	 * 如复选，则返回TreeNode.attributes的Array<br>
	 * 不会查找未加载的节点<br>
	 */
...
Ext.reg("ssc.component.baselisttgfield", ssc.component.BaseListField);