...
/*	iconCls : "xy-grid",*/
	...
	getFormData : function()
	{
		...
		for (var i = 0; i < this.getStore().data.length; i++)
		{
			...
			for (var m = 1; m < this.colModel.getColumnCount(); m++)
			{
				if (this.colModel.config[m].XyNotSave === true)
				{
					continue;
				}

				var fieldName = this.colModel.config[m]["XySaveField"] ? this.colModel.config[m]["XySaveField"]
					: this.colModel.getDataIndex(m);

				var fieldValue = "";
				if (this.colModel.config[m].XySaveType == "datefield")
				{
					if (typeof (record.get(this.colModel.getDataIndex(m)).format) == "undefined")
					{
						fieldValue = record.get(this.colModel.getDataIndex(m));
					}
					else
					{
						fieldValue = record.get(this.colModel.getDataIndex(m)).format("Y-m-d");
					}
				}
				else if (this.colModel.config[m].XySaveType == "moneyfield")
				{
					fieldValue = record.getXyValue(this.colModel.getDataIndex(m)).toFixed(2);
				}
				else
				{
					...
					{
						var _val = record.getXyValue(this.colModel.getDataIndex(m));
						fieldValue = _val == null ? "" : _val;
					}
				}

				entityData[fieldName] = fieldValue;
			}

			detailData.push(entityData);
		}
