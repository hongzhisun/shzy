...
{
	var dsItems = new Ext.data.JsonStore({
	...
	var clnExpander = new Ext.grid.RowExpander({tpl : new Ext.Template("<p><b>详细信息:</b><br><br>{checkdesc}</p><br>")});
	...
	var cm = new Ext.grid.ColumnModel([clnExpander,clnCheckID, clnStep, clnAc, clnUser, clnDesc, clnTime, clnType]);

	var sm = new Ext.grid.RowSelectionModel({singleSelect : true});
	
	var grdChk = new Ext.grid.GridPanel({
				store: dsItems,
				border: false,
				colModel: cm,
	...
	function loadException(This, node, response)
	{
		...
		switch (status)
		{
			...
			case 200 :
				if (text.length > 0)
				{
					Ext.MessageBox.alert("加载数据时发生以下错误", text);
				}
				break;
			default :
				Ext.MessageBox.alert("加载数据时发生以下错误", status + "," + text);
				break;
		}
	}	
}